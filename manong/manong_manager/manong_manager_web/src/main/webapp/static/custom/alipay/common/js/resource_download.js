...
$(function() {
	...
	var turn = function(target, time, opts) {
		target.find('.cura').hover(function() {
			$(this).find('.in_1').eq(0).stop().animate(opts[0], time, function() {
				$(this).hide().next().show();
...
$(document).ready(function() {
	...
	$tab_li.hover(function() {
		$(this).addClass('select').siblings().removeClass('select');
		var index = $tab_li.index(this);
		$('div.hot_list > div').eq(index).show().siblings().hide();
	});
});
//radio选择
$(function() {
...
//左侧栏展开
$(function() {
	$('.problem_content_left .nav_box').bind('click', function() {
		if($(this).hasClass('unfold')) {
			$(this).removeClass('unfold').find('h3.nav_til').find('span').eq(1).attr('class', 'til_arr_up');
		} else {
			$(this).addClass('unfold').find('h3.nav_til').find('span').eq(1).attr('class', 'til_arr_down');
		}
	})
